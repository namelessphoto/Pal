...
APalAIController::APalAIController(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer.SetDefaultSubobjectClass<UPalFollowingComponent>(TEXT("PathFollowingComponent")).SetDefaultSubobjectClass<UPalAIActionComponent>(TEXT("ActionsComp"))) {
    ...
    this->IsSelfTimeDelate_FromSpawnTime_ForVisitor = false;
...
TEnumAsByte<EPathFollowingRequestResult> APalAIController::PalMoveToLocation(const FVector& Dest, float AcceptanceRadius, bool bStopOnOverlap, bool bUsePathfinding, bool bProjectDestinationToNavigation, bool bCanStrafe, TSubclassOf<UNavigationQueryFilter> FilterClass, bool bAllowPartialPaths) {
